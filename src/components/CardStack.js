...
const toValue = i => ({ x: 0, y: i * -1, scale: 1, rot: Math.random() * 3, delay: i * 50 })
...
const Deck = ({ cards, currentClickId }) => {
  ...
  const bind = useDrag(({ args: [index], down, delta: [xDelta], distance, direction: [xDir], velocity }) => {
    ...
    set(i => {
      ...
      return { x, rot, scale, delay: undefined, config: { friction: 40, tension: down ? 800 : isGone ? 200 : 500 } }
    })
    if (!down && gone.size === cards.length) setTimeout(() => gone.clear() || set(i => toValue(i)), 500)
  })
  return props.map(({ x, y, rot, scale }, i) => (
    <animated.div className={'SliderCards'} key={i} style={{ transform: to([x, y], (x, y) => `translate3d(${x}px,${y}px,0)`) }}>
      {/* This is the card itself, we're binding our gesture to it (and inject its index so we know which is which) */}
      <animated.div {...bind(i)} style={{ transform: to([rot, scale], trans), backgroundImage: `url(${cards[i].file.url})` }} />
    </animated.div>
  ))
}
